-- Modificaciones para el sistema de producción industrial

-- 1. Actualizar tabla usuarios para usar cédula en lugar de email
ALTER TABLE public.usuarios 
DROP COLUMN IF EXISTS auth_user_id,
ADD COLUMN IF NOT EXISTS cedula TEXT UNIQUE NOT NULL DEFAULT '',
ADD COLUMN IF NOT EXISTS password_hash TEXT NOT NULL DEFAULT '';

-- 2. Crear enum para turnos
DO $$ BEGIN
...
-- 3. Modificar tabla registros_produccion
...
-- Cambiar el tipo de turno si no es el correcto
ALTER TABLE public.registros_produccion 
...
-- 7. Políticas RLS para detalle_produccion
CREATE POLICY "Todos pueden ver detalles de producción" 
ON public.detalle_produccion 
FOR SELECT 
USING (true);

CREATE POLICY "Operarios pueden insertar detalles de producción" 
ON public.detalle_produccion 
FOR INSERT 
WITH CHECK (
    EXISTS (
        SELECT 1 FROM public.registros_produccion rp
        JOIN public.usuarios u ON u.id = rp.operario_id
        WHERE rp.id = detalle_produccion.registro_id
        AND u.activo = true
    )
);
...
-- 8. Políticas RLS para registro_asistentes
CREATE POLICY "Todos pueden ver registro asistentes" 
...
WITH CHECK (
    EXISTS (
        SELECT 1 FROM public.registros_produccion rp
        JOIN public.usuarios u ON u.id = rp.operario_id
        WHERE rp.id = registro_asistentes.registro_id
        AND u.activo = true
    )
);
